...
用户说「推送」（或同等发布意图）时：**先打印含实际分支名、提交信息、命令的步骤计划，等用户确认后再执行**。不要直接执行。

前提：在功能分支开发（如 `company`）。目标主分支为 `master`。开始前记下当前分支名，全部完成后切回去。

提交信息一律用中文。远程 `master` 有未拉取提交则先 `git pull origin master`；冲突谨慎，自己能处理的处理完要说明，不能处理的停下来问。不修改 git config、不强推、不跳过 hook。当前 shell 为 PowerShell，多条命令用 `;` 连接，不用 `&&`。
...
3. **功能代码提交**：其余改动（主要 `src/**`、规则/技能等）再提交一次。
...
- 当前分支、完成后切回的分支名
- 每类改动的实际文件路径、是否进 master
- 将执行的命令与中文提交信息
- 无某类改动则跳过对应提交，并在计划中说明

## 注意

- A 类永远不进 `master`、不推远程，仅作为功能分支上的本地快照
- B 类与代码一样进 `master` 并推远程；计划中要单独点名这些文件与提交
- 上一需求已完成、用户又提新需求，且工作区仍有待发布改动时：先按本流程出计划（等同「推送」），确认并完成后再做新需求
