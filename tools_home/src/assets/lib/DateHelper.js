class DateHelper {
  constructor() {
    ...
    //农历节日
    this.lunarFestival = [{ "month": "1", "date": "1", "name": "春节" }, { "month": "1", "date": "15", "name": "元宵节" }, { "month": "5", "date": "5", "name": "端午节" }, { "month": "7", "date": "7", "name": "七夕节" }, { "month": "8", "date": "15", "name": "中秋节" }, { "month": "9", "date": "9", "name": "重阳节" }, { "month": "12", "date": "8", "name": "腊八节" }]
    //公历节日
    this.gregorianFestival = [{ "month": "1", "date": "1", "name": "元旦" }, { "month": "3", "date": "8", "name": "妇女节" }, { "month": "3", "date": "12", "name": "植树节" }, { "month": "5", "date": "1", "name": "劳动节" }, { "month": "5", "date": "4", "name": "青年节" }, { "month": "6", "date": "1", "name": "儿童节" }, { "month": "7", "date": "1", "name": "建党节" }, { "month": "8", "date": "1", "name": "建军节" }, { "month": "9", "date": "10", "name": "教师节" }, { "month": "10", "date": "1", "name": "国庆节" }];
  ...
  isAllowLunar(dateElements) {
    if (dateElements.fullYear < this.MIN_LUNAR_YEAR || dateElements.fullYear > this.MAX_LUNAR_YEAR) {
      return false;
    }
    if (dateElements.fullYear == this.MIN_LUNAR_YEAR && dateElements.month == this.MIN_LUNAR_MONTH && dateElements.date < this.MIN_LUNAR_DAY) {
      return false;
    }
  ...
  getLunarDate(argDate, isAllString) {
    ...
    if (this.isAllowLunar(dateElements)) {
      ...
      var offset = (Date.UTC(dateElements.fullYear, dateElements.month - 1, dateElements.date) - Date.UTC(this.MIN_LUNAR_YEAR, this.MIN_LUNAR_MONTH - 1, this.MIN_LUNAR_DAY)) / 86400000;
      var i, temp;
  ...
  getLunarCountDay(year) {
    var i, sum = 348;
    for (i = 0x8000; i > 0x8; i >>= 1) { sum += (this.lunarInfo[year - this.MIN_LUNAR_YEAR] & i) ? 1 : 0; }
  ...
  getFestivalNotice(originalDate) {
    ...
    put_notice_nextFestival(this.lunarFestival, lunarDateElements, notic.nextFestival, this.MIN_NOTICE_DAY);
    put_notice_nextFestival(this.gregorianFestival, dateElements, notic.nextFestival, this.MIN_NOTICE_DAY);
    put_notice_currentFestival(this.lunarFestival, lunarDateElements, notic.currentFestival)
    put_notice_currentFestival(this.gregorianFestival, dateElements, notic.currentFestival);
    put_notice_holidayFestival(this.publicHoliday, dateElements, notic.holidayFestival);
    put_notice_addFestival(this.publicHoliday, dateElements, notic.addFestival);
    put_notice_nextAddFestival(this.publicHoliday, dateElements, notic.nextAddFestival, this.MIN_NOTICE_ADD_DAY);
    function put_notice_nextAddFestival(festival_1, dateElements_1, notic_1, MIN_NOTICE_ADD_DAY) {
      let nextDateElements = dh.getElements(false, dh.getNextDay(dateElements_1.original, MIN_NOTICE_ADD_DAY));
      if (festival_1[dateElements_1.fullYear]) {
        festival_1[dateElements_1.fullYear].forEach((el) => {
          if (el.add[0]) {
            if (el.add[0].month == nextDateElements.month && el.add[0].date == nextDateElements.date) {
              notic_1.push(el);
            }
          }
        })
      ...
      if (festival_1[dateElements_1.fullYear]) {
        festival_1[dateElements_1.fullYear].forEach((el) => {
          let index = el.add.findIndex((el_1) => {
            if (el_1.month == dateElements_1.month && el_1.date == dateElements_1.date) {
              return true;
            }
          });
          if (~index) {

            notic_1[index] && notic_1[index].push(el) || (notic_1[index] = []).push(el);
          }
        })
      }
    }
    ...
    function put_notice_currentFestival(festival_1, dateElements_1, notic_1) {
      festival_1.forEach((el) => {
        if (el.month == dateElements_1.month && el.date == dateElements_1.date) {
          notic_1.push(el)
        }
      })
    }
    function put_notice_nextFestival(festival_1, dateElements_1, notic_1, MIN_NOTICE_DAY_1) {
      festival_1.forEach((el) => {
        let days = [], originalDate_festival = dh.getOriginalDate(dateElements_1.fullYear, el.month, el.date);
        ...
        let day = days.findIndex((el_1) => {
          ...
          if (dateElements_1.month == dateElements.month && dateElements_1.date == dateElements.date) {
            return true;
          }
        })
        ...
        if (~day) {
          ...
          notic_1[day] && notic_1[day].push(el) || (notic_1[day] = []).push(el);
