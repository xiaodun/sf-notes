(function () {
  return function (argData, argParams) {
    ...
    if (platform === "darwin") {
      return openTerminalOnMac(argData, argParams);
    }
    // Default: Windows
    return openTerminalOnWin(argData, argParams);
  };

  function openTerminalOnWin(argData, argParams) {
    const fs = require("fs");
  ...
  function openTerminalOnMac(argData, argParams) {
    const fs = require("fs");
    const path = require("path");
    const { exec, execSync } = require("child_process");
    const os = require("os");
